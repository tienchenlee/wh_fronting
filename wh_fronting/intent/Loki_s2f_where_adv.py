...
def getResult(inputSTR, utterance, args, resultDICT, refDICT, pattern="", toolkitDICT={}):
    ...
    if utterance == "他們在台北見過面":
        if CHATBOT:
            ...
            resultDICT["FT"].append("在哪裡" + inputSTR.replace(args[0], "").replace(args[1], ""))


    if utterance == "張三不喜歡台北":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
            ...
            resultDICT["FT"].append("哪裡" + inputSTR.replace(args[0], ""))


    if utterance == "張三在台北看到瑪麗":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
...
if __name__ == "__main__":
    ...
    resultDICT = getResult("張三不喜歡台北", "張三不喜歡台北", [], {}, {})
    pprint(resultDICT)