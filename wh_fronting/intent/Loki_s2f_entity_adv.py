...
"""
    Loki module for s2f_entity_adv
...
def getResult(inputSTR, utterance, args, resultDICT, refDICT, pattern="", toolkitDICT={}):
    ...
    if utterance == "張三一定會買衛生紙":
        if CHATBOT:
            ...
            resultDICT["FT"].append("什麼東西" + inputSTR.replace(args[0], ""))


    if utterance == "張三不買中國貨":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
...
if __name__ == "__main__":
    ...
    resultDICT = getResult("張三不買中國貨", "張三不買中國貨", [], {}, {})
    pprint(resultDICT)